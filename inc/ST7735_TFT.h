...
static void st7735_fill_start() {
  ...
  spi_data(0x00, 0x00, 0x00, 0x9F);  // 0..159 landscape width

  spi_command(ST7735_RASET);
  spi_data(0x00, 0x00, 0x00, 0x7F);  // 0..127 landscape height
...
static void st7735_fill_send(uint16_t pixel) {
  uint8_t buf[2] = { (uint8_t)(pixel >> 8), (uint8_t)(pixel & 0xFF) };
  spi_write_blocking(SPI_TFT_PORT, buf, 2);
}
...
static void st7735_init() {
  ...
  // read screen data sheet to understand
  {
    ...
    write_data(0x20 | 0x40 | ST7735_MADCTL_RGB); // MV | MX | RGB: landscape 160x128
    write_command(ST7735_COLMOD);
    write_data(0x05); 
  }

  { // initializes red version — landscape 160x128
    write_command(ST7735_CASET);
    write_data(0x00); write_data(0x00);
    write_data(0x00); write_data(0x9F); // 0..159 landscape width
    write_command(ST7735_RASET);
    write_data(0x00); write_data(0x00);
    write_data(0x00); write_data(0x7F); // 0..127 landscape height
  }
